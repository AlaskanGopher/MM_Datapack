# Stop all animations
function animated_java:south_gate_soldier/animations/soldier_idle/pause
function animated_java:south_gate_soldier/animations/soldier_block/pause
function animated_java:south_gate_soldier/animations/soldier_move/pause

# Store Current
execute store result score @s coord_x run data get entity @s Pos[0] 100

# Animation
execute unless entity @a[sort=nearest,x=3,y=100,z=56,dx=-6,dy=1,dz=-3] if entity @s[x=0,y=100,z=57.5] if entity @e[tag=globals,tag=!GatesOpen] run function animated_java:south_gate_soldier/animations/soldier_idle/resume
execute if entity @a[sort=nearest,x=3,y=100,z=56,dx=-6,dy=1,dz=-3] if score @s coord_x = @s last_coord_x if entity @e[tag=globals,tag=!GatesOpen] run function animated_java:south_gate_soldier/animations/soldier_block/resume
execute if entity @a[sort=nearest,x=3,y=100,z=56,dx=-6,dy=1,dz=-3] unless score @s coord_x = @s last_coord_x if entity @e[tag=globals,tag=!GatesOpen] run function animated_java:south_gate_soldier/animations/soldier_move/resume
execute if entity @e[tag=globals,tag=GatesOpen] run function animated_java:south_gate_soldier/animations/soldier_idle/resume
