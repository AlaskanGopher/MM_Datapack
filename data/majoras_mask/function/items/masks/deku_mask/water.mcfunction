...
#$execute if score @s WorkingVariable matches 1 unless entity @e[tag=$(id)] as @s at @s rotated ~ 0 run summon happy_ghast ^ ^-4.1 ^ {NoAI: 1b, Invulnerable:1b, Tags:["$(id)"]}
#$execute if score @s WorkingVariable matches 0 if entity @e[tag=$(id)] run kill @e[tag=$(id)]

#execute at @s positioned ^ ^ ^ run summon wind_charge ~ ~ ~
#execute at @s rotated ~ 0 run tp @s ^ ^3 ^